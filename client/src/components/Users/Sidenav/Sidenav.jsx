...
const Sidenav = () => {
  ...
  // Get and decode token if available
  ...
  if (accessToken) {
    ...
    user_type = null; // Handle the case when the user is not logged in
  }

  return (
    <>
      <nav
        className={`font-noto ${((!isHoverOn && isSidebarOpen) || isHoverOn) ? "lg:hover:w-[16em]" : "lg:w-20 xm:w-0 sm:w-0"} z-50 fixed top-0 h-screen bg-white pt-4 text-black shadow-outerShadow ${isSidebarOpen ? "w-[16em]" : "lg:w-20"} ease-linear duration-200 overflow-auto`}
        onMouseEnter={hoverOn}
        onMouseLeave={hoverOff}
      >
        <div
          className={`justify-end items-center text-block ${isHoverOn || isSidebarOpen ? "flex" : "hidden"}`}
          onClick={toggleSidebar}
        >
          <FontAwesomeIcon icon={faXmark} className="xm:text-xl pr-3" />
        </div>
        <div className={`flex-col ${!isSidebarOpen && !isHoverOn ? "mt-20" : "mt-0"} ease-linear duration-300`}>
