SEDM_CURRICULUM = [
    # First Year First Semester
    {
        "program": 14,
        "department": 5,
        "semester": "FIRST_SEMESTER",
        "year_level": 1,
        "courses": [
            {"course": "MAT101"},  # Mathematics in the Modern World
            {"course": "PSY100"},  # Understanding the Self
            {"course": "READ100"},  # Science and Development of Reading
            {"course": "ITE101"},  # Living in the IT Era
            {"course": "SMM100"},  # History of Mathematics
            {"course": "SMM101"},  # College and Advanced Algebra
            {"course": "PHD101"},  # PnCians Holistic Development 1
            {"course": "PED101"},  # PATHFit 1: Movement Competency Training MCT
            {"course": "NSTP1"},  # National Service Training Program 1
        ],
    },
    # First Year Second Semester
    {
        "program": 14,
        "department": 5,
        "semester": "SECOND_SEMESTER",
        "year_level": 1,
        "courses": [
            {"course": "GAD101"},  # Gender and Development
            {"course": "HIS101"},  # Readings in Philippine History
            {"course": "HMN101"},  # Art Appreciation
            {"course": "COM101"},  # Purposive Communication
            {"course": "SMM102"},  # Trigonometry
            {
                "course": "CED101"
            },  # The Child and Adolescent Learners and Learning Principles
            {"course": "CED102"},  # Technology for Teaching and Learning 1
            {"course": "PHD102"},  # PnCians Holistic Development 2
            {"course": "PED102"},  # PATHFit 2: Exercise-Based Fitness Activities
            {"course": "NSTP2"},  # National Service Training Program 2
        ],
    },
    # Second Year First Semester
    {
        "program": 14,
        "department": 5,
        "semester": "FIRST_SEMESTER",
        "year_level": 5,
        "courses": [
            {"course": "STS101"},  # Science, Technology and Society
            {"course": "SOC101"},  # The Contemporary World
            {"course": "RIZ101"},  # Life and Works of Rizal
            {"course": "ENT101"},  # The Entrepreneurial Mind
            {"course": "ETH101"},  # Ethics
            {"course": "ENV101"},  # Environmental Science
            {"course": "SMM103"},  # Plane and Solid Geometry
            {"course": "SMM104"},  # Logic and Set Theory
            {
                "course": "PED103"
            },  # PATHFit 3: Choice of Dance, Sports, Martial Arts, Group Exercise, Outdoor, and Adventure Activities
        ],
    },
    # Second Year Second Semester
    {
        "program": 14,
        "department": 5,
        "semester": "SECOND_SEMESTER",
        "year_level": 5,
        "courses": [
            {
                "course": "CED103"
            },  # The Teacher and the Community, School Culture, and Organizational Leadership
            {"course": "CED104"},  # Foundation of Special and Inclusive Education
            {"course": "CED105"},  # Facilitating Learner-Centered Teaching
            {"course": "CED106"},  # Assessment in Learning 1
            {"course": "CED108"},  # The Teaching Profession
            {"course": "SMM105"},  # Elementary Statistics & Probability
            {"course": "SMM107"},  # Mathematics of Investment
            {"course": "SMM108"},  # Calculus 1
            {
                "course": "PED104"
            },  # PATHFit 4: Choice of Dance, Sports, Martial Arts, Group Exercise, Outdoor, and Adventure Activities
        ],
    },
    # Third Year First Semester
    {
        "program": 14,
        "department": 5,
        "semester": "FIRST_SEMESTER",
        "year_level": 3,
        "courses": [
            {"course": "CED107"},  # Assessment in Learning 2
            {"course": "CED109"},  # The Teacher and the School Curriculum
            {"course": "SMM109"},  # Abstract Algebra
            {"course": "SMM116"},  # Modern Geometry
            {
                "course": "SMM113"
            },  # Problem Solving, Mathematical Investigation, and Modelling
            {"course": "SMM106"},  # Principles and Strategies of Teaching Mathematics
            {"course": "SMM112"},  # Advanced Statistics
            {"course": "SMM111"},  # Calculus 2
        ],
    },
    # Third Year Second Semester
    {
        "program": 14,
        "department": 5,
        "semester": "SECOND_SEMESTER",
        "year_level": 3,
        "courses": [
            {
                "course": "CED110"
            },  # Building and Enhancing New Literacies Across the Curriculum
            {"course": "SMM115"},  # Calculus 3
            {"course": "SMM110"},  # Linear Algebra
            {"course": "SMM117"},  # Number Theory
            {"course": "SMM118"},  # Research in Mathematics
            {
                "course": "SMM114"
            },  # Technology for Teaching and Learning 2 - Instrumentation and Technology in Mathematics
            {"course": "SMM119"},  # Assessment and Evaluation in Mathematics
        ],
    },
    # Fourth Year First Semester
    {
        "program": 14,
        "department": 5,
        "semester": "FIRST_SEMESTER",
        "year_level": 4,
        "courses": [
            {"course": "CED111"},  # Field Study 1
            {"course": "CED112"},  # Field Study 2
            {
                "course": "CED201"
            },  # Competency Appraisal 1 (General Education/Specialization)
        ],
    },
    # Fourth Year Second Semester
    {
        "program": 14,
        "department": 5,
        "semester": "SECOND_SEMESTER",
        "year_level": 4,
        "courses": [
            {"course": "CED113"},  # Teaching Internship
            {"course": "CED202"},  # Competency Appraisal 2 (Professional Education)
        ],
    },
]
