...
const Sidenav = () => {
  ...
  // State to hold decoded token data
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Failed to decode token or access localStorage:", error);
    }
  }, []);
  ...
  return (
    <>
      ...
      >
        <div className={`justify-end items-center text-block ${isHoverOn || isSidebarOpen ? "flex" : "hidden"}`} onClick={toggleSidebar}>
        ...
        <div className={`flex-col ${!isSidebarOpen && !isHoverOn ? "mt-20" : "mt-0"} ease-linear duration-300`}>
          ...
          <ul className="flex flex-col gap-2 px-4">
            {userType === "P" ? (
              ...
              <li className="w-full">
                <Link to="vpaa" className="flex w-full items-center rounded-lg px-1 py-2 hover:bg-gray-200">
                  <FontAwesomeIcon icon={faGears} className={`${!isSidebarOpen ? "xm:hidden sm:hidden lg:flex" : ""}`} />
                  <span className={`text-sidenavTextSize ml-3 ${((!isHoverOn && isSidebarOpen) || isHoverOn) ? "flex" : "hidden"}`}>Schedule</span>
                </Link>
              </li>
